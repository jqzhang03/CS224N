...
\begin{parts}
    ...
    \ifans{
        ...
        \begin{tabular}{ p{4.2cm} | p{4.2cm} | l | l}
        Stack & Buffer & New dependency & Transition \\ \hline
        [ROOT] & [I, presented, my, findings, at, the, NLP, conference] &  & Initial Configuration \\
        $[$ROOT, I] & [presented, my, findings, at, the, NLP, conference] &  &  \texttt{SHIFT}  \\
        $[$ROOT, I, presented] & [my, findings, at, the, NLP, conference] &  &  \texttt{SHIFT}  \\
    ...
    \ifans{
        \newline
        整体操作次数为$O(n)$，其中执行一次SHIFT操作需要一个步骤，执行一次LEFT-ARC操作需要两个步骤，执行一次RIGHT-ARC操作需要两个步骤，整体复杂度为$O(n)$。
    }
...
\begin{itemize}
    ...
    \item 介词短语附着错误：在上面的例子当中，短语"into Afghanistan"是一个介词短语，该介词短语被错误地连接到了错误的中心词，这个例子的意思是莫斯科向阿富汗派遣军队，那么"into Afghanistan"是修饰动词"sent"的，而不是修饰军队"troops"
    ...
    \item 动词短语附着错误：在本例当中，\textit{leaving the store unattended}意思是让商店无人看管，该动词短语被错误的连接到错误的中心词上，正确的中心词应该是\textit{went}。
    ...
    \item 修饰语附着错误：该例子中，"extremely"是"short"的修饰语，修饰语附着错误指的就是一个修饰语被错误地连接到中心词上
    ...
    \item 并列附着错误：例子中短语"brown rice"和"garlic naan"都是并列成分，单词"or"是并列连词，第二个并列成分应该连接到第一个并列成分
\end{itemize}
...
\ifans{
    ...
    Part-of-speech(POS) tags为每一个单词都提供了有效的语法信息，让大模型能够更清楚地了解一段话的语义结构。
    POS tags能够帮助大模型在不同的上下文中，理解每个单词的含义，即便一个单词可能存在多种意思。
    通过加入上下文，可以提升解析器对名词、动词、介词、修饰词等语法的理解，进而提高解析效率。
}
