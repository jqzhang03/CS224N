...
注意力计算公式为：
$$
    \alpha_{t, i} = \frac{\exp(\be_{t,i})}{\sum_{1 \le j \le m} \exp(\be_{t, j})}
$$
其中$\be_{t, i}$指的是第t个解码隐藏状态对第i个编码隐藏状态的注意力分数，如果$\be_{t, i}$设置为$-\infty$，那么注意力权重将在softmax后变成0。

对编码隐藏状态进行掩码处理，以防止查询向量(第t个解码隐藏状态)关注到填充的部分。
}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{images/QQ20260323-181545.png}
\end{figure}
...
点乘注意力包含向量之间的直接点乘操作，使其运算更加高效和速度非常快，但它缺少了额外的预训练参数，对于非常复杂的上下文无法注意到更多的位置信息，使得模型缺乏灵活性。
}
...
加法注意力引入了一个非线性变换，使其对于捕捉query和key之间的复杂关系时更加灵活，但加法的运算复杂度增加了，在某些例子中可能会导致过拟合，比如有限的数据。
}
\end{subparts}

{\color{red}训练说明：

在模型训练的时候，可以选择autodl、恒源云等平台租用显卡，使用的时候要看好自己要求的cuda版本以及对应显卡适合的cuda版本，将自己的代码打包后导入到服务器上，
然后再通过requirement.txt安装各种环境依赖。
}
