...
\begin{subparts}
    ...
    \ifans{
        要想$\alpha_j \gg \sum_{i \neq j} \alpha_i$，从

        $$
        \alpha_i = \frac{\exp(k_i^\top q)}{\sum_{j=1}^{n} \exp(k_j^\top q)}
        $$

        可得$k_j^\top q \gg k_i^\top q, \quad \forall i \in \{1, \ldots, n\} \quad \text{and} \quad i \ne j$
    }

    \subpart[1] Under the conditions you gave in (i), \textbf{describe} the output $c$. 

    \ifans{
        因为$\sum_{i=1}^{n} \alpha_{i} = 1$，并且$\alpha_{j} \approx 1$，因此可以得到
...
\ifans{
    ...
    意味着$k_{a}^\top q \approx k_{b}^\top q \gg k_{i}^\top q, \quad \forall i \in {1, \ldots, n}, \text{and} \quad i \neq a, \text{and} \quad i \neq b$

    也就是说$q = \beta (k_a + k_b), \quad \beta \gg n > 0$

    那么

    \begin{align*}
        k_{a}^\top q &= \beta (k_{a}^\top k_{a} + k_{a}^\top k_{b}) = \beta (1 + 0) = \beta
        k_{b}^\top q &= \beta (k_{b}^\top k_{a} + k_{b}^\top k_{b}) = \beta (0 + 1) = \beta
        k_{i}^\top q &= \beta (k_{i}^\top k_{a} + k_{i}^\top k_{b}) = \beta (0 + 0) = 0, \quad \forall i \neq a \quad \text{and} \quad i \neq b
    \end{align*}
    ...
    \begin{align*}
        ...
        &= \sum_{i = 1} ^{n} v_{i} \frac{\exp(k_{i}^\top q)}{\sum_{j = 1}^{n} \exp(k_{j}^\top q)}
        &= \sum_{i = 1} ^{n} v_{i} \frac{\exp(k_{i}^\top q)}{n - 2 + 2\beta}
        &\approx v_{a} \frac{\beta}{2\beta} + v_{b} \frac{\beta}{2\beta}
        &= \frac{1}{2}(v_{a} + v_{b})
    \end{align*}
...
\ifans{
为了使得注意力输出$c \approx \frac{1}{2}(v_a + v_b)$，令查询向量$q = \beta(\mu_a + \mu_b)$，其中$\beta \gg 1$，保证在n比较大的时候其他项依然趋于0

由于协方差矩阵$\Sigma_i = \alpha I$中$\alpha$趋近于0，随机采样的k就会分布在$\mu$的周围，那么可以近似地认为$k_i \approx \mu_i$，注意力分数$score_i=q^\top k_i \approx q^\top \mu_i$，与上一个问题同理，因为向量之间两两正交，且模长都为1，
因此$q^\top \mu$只有当$\mu$为a和b的时候才会有值，其他情况均为0，由于a和b的注意力分数等大，且远大于其他向量的注意力分数，因此最后Softmax函数后，a和b平均分总权重，其他向量权重接近于0。
}
...
\ifans{
在本题当中，$k_a$的模长会产生一定的变化，使得$c$不在近似于稳定的$\frac{1}{2}(v_a + v_b)$，而是在$v_a$和$v_b$之间抖动，当$k_a$模长比较大的时候，$c$会更偏向于$v_a$，当$k_a$模长比较小的时候，$c$会更偏向于$v_b$。

根据注意力分数公式$score_i=q^\top k_i$，$k_b$的分数基本恒定，但$k_a$的分数会在一定范围内跳动，并且协方差$\Sigma_a = \alpha I + \frac{1}{2}(\mu_a\mu_a^\top)$多了一个$\frac{1}{2}(\mu_a\mu_a^\top)$，意味着$k_a$在$\mu_a$方向上的投影长度会有显著的方差。

最后Softmax函数后，由于a和b的注意力分数不再近似，那么注意力权重也会随着$k_a$模长的变化而变化。
...
因为向量之间两两正交，且向量的模长都是1，在多头注意力机制下，$q_1$和$q_2$会进入不同的头进行计算，头1的输出$c_1 \approx v_a$，头2的输出$c_2 \approx v_b$，多头注意力的输出是各个头的平均值，因此最终输出$c \approx \frac{1}{2}(v_a + v_b)$。

在这种情况下，头1只会关注a，头2只会关注b，因此如果$k_a$的模长不稳定像前面单头注意力那样，也不会对头2中$k_b$的注意力分数产生影响。
}
...
\ifans{
根据$\Sigma_a=\alpha I + \frac{1}{2}(\mu_a\mu_a^\top)$，其中$q_1=\mu_a$专门针对a，那么$k_a$的协方差会在$\mu_a$方向上有较大的方差，导致$k_a$的模长产生波动。

而对于头2来说，其主要关注$q_2=\mu_b$，$\k_a$的扰动在b方向上为0，因为向量之间两两正交的关系，无论$k_a$模长怎么变，都不会改变其在$\mu_b$方向上的投影，因此$c_2 \approx v_b$依然是稳定的。

最后在Softmax函数中，头1虽然受到$k_a$的扰动，但其不关注$k_b$，头2才会关注$k_b$，因此即使$c_1$有轻微的扰动，$c_2$依然能够稳固提供$v_b$的信息，因此$c \approx \frac{1}{2}(v_a + v_b)$
}
\end{subparts}
...
\ifans{
多头注意力机制通过将不同的查询向量分发到各个头，使得每个头能够专注于一个向量，尽管某些向量会因为协方差矩阵或其他原因导致模长的变化，但由于向量之间两两正交的关系，并不会影响其他方向上的向量，在计算注意力权重时，每个头平均分配权重，
而不会因为某些向量的扰动而发生剧烈变化。

单头注意力机制由于所有的查询向量都在一个头上，当某个向量模长发生变化，其会更偏向于该方向，最终导致注意力权重分配时影响其他向量的权重。
}
